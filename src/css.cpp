...
{
    bool isDiscontinuous[cloud.size()] = {false};
    float Dmax;
    float ri_1;
    float deltaPhi;
    float stdDev = 0.02;
    float lambda = 9.0 * M_PI / 180.0;
    pcl::PointXYZI pi,pi_1;
    int numClusters = 0;
    ...
    {
      pi_1.x = (cloud)[(i-1)%cloud.size()].x;
      pi_1.y = 0;
      pi_1.z = (cloud)[(i-1)%cloud.size()].z;
      pi_1.intensity = (cloud)[(i-1)%cloud.size()].intensity;

      pi.x = (cloud)[i%cloud.size()].x;
      pi.y = 0;
      pi.z = (cloud)[i%cloud.size()].z;
      pi.intensity = (cloud)[i%cloud.size()].intensity;


      Vector3f vi_1(pi_1.x, pi_1.y, pi_1.z);
      Vector3f vi(pi.x, pi.y, pi.z);
      deltaPhi = atan2( (vi.cross(vi_1)).norm(), vi.dot(vi_1) );

      ri_1 = sqrt( pi_1.x*pi_1.x + pi_1.z*pi_1.z );
      Dmax = ri_1 * (sin(deltaPhi)/sin(lambda-deltaPhi)) + 3.0*stdDev;

        //if(pcl::euclideanDistance(pi_1,pi)>Dmax)
      //if(pcl::euclideanDistance(pi_1,p1)-Dmax>0.01)
      if(pcl::euclideanDistance(pi,pi_1)>Dmax)
      {
        isDiscontinuous[i%cloud.size()] = true;
        numClusters++;
      }else {
        isDiscontinuous[i%cloud.size()] = false;
      }
    }
